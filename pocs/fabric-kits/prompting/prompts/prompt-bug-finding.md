---
id: prompt-bug-finding
type: rules
name: prompt bug finding
description: Behavioral defect discovery methodology for prompts and agent instructions; companion rules loaded by prompt-review
---

<!-- append "bug_finding_heading" -->
# Prompt Bug-Finding Methodology
<!-- /append -->

<!-- append "bug_finding_scope" -->
...
<!-- append "bug_finding_core_principles" -->
## Core Principles
...
- Load only the instructions needed for the active execution path, but do not treat an uninspected dependency as irrelevant. When a reference may affect routing, authority, safety, state, recovery, or output behavior, apply the bounded dependency escalation policy defined in `bug_finding_dependency_escalation`.
...
<!-- append "bug_finding_dependency_escalation" -->
## Bounded Dependency Escalation

- Start with `1` slice from `1` dependency file.
- Define a `slice` as one contiguous excerpt from one dependency file: one TOC read, one section, or one contiguous line range.
- A TOC read counts as one slice only when the inspected TOC excerpt itself fits the slice budget; if the TOC is longer than the budget, narrow it to the smallest contiguous TOC subsection or line range that can still resolve the question.
- Use file metadata, section numbering, heading ranges, and targeted keyword searches to identify the smallest decisive contiguous excerpt.
- Do not merge disjoint excerpts and count them as one slice; if a section or TOC excerpt is longer than the budget, narrow it to the smallest contiguous subsection or range that can still resolve the question.
- Keep each slice to `<= 120` raw lines, keep at most `3` dependency files and `<= 400` raw dependency lines in active review context, and summarize each decisive slice into the retained working set before further escalation.
- Escalate only while the dependency remains materially unresolved.
- If the next escalation would exceed the budget or still requires whole-file loading, stop, checkpoint the unresolved dependency set, and mark the review `PARTIAL`.
- In interactive mode, ask the user whether to expand scope or continue in a follow-up review.
- In non-interactive or CI-style execution, do not block on that question: emit the checkpoint plus the exact additional scope required for the next pass.
<!-- /append -->

<!-- append "bug_finding_layer_map" -->
## Layer Map
...
<!-- append "bug_finding_l1_hotspots" -->
...
<!-- append "bug_finding_l2_contracts" -->
...
<!-- append "bug_finding_l3_branches" -->
...
<!-- append "bug_finding_l4_bug_classes" -->
...
<!-- append "bug_finding_l5_counterexamples" -->
...
<!-- append "bug_finding_l6_validation" -->
...
<!-- append "bug_finding_l7_reporting_summary" -->
...
<!-- append "bug_finding_l7_status_semantics" -->
Status semantics:
...
<!-- append "bug_finding_l7_finding_format" -->
...
<!-- append "bug_finding_l7_residual_uncertainty" -->
...
<!-- append "bug_finding_execution_sequence" -->
## Execution Protocol
...
<!-- append "bug_finding_efficiency_rules" -->
Efficiency rules:

- Prefer narrow prompt slices over loading the full instruction stack.
- Apply `bug_finding_dependency_escalation` whenever a dependency may affect routing, authority, safety, state, recovery, or output behavior.
- Prefer TOC, section, or range reads over whole-file loading.
...
<!-- append "bug_finding_validation_checklist" -->
## Validation
