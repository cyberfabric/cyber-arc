...
type: rules
name: prompt script
description: Author a fabric script (JS module with id, name, description, interface, run) and register it alongside the prompts that invoke it
---

<!-- append "script_intro" -->
Use fabric prompt script authoring mode. This mode produces a JavaScript file under a kit's `scripts/` directory — not a markdown prompt.
<!-- /append -->

<!-- append "script_goal" -->
1. The goal of this mode is to produce a fabric script that is registerable, loadable, deterministic, and self-describing. Scripts hold behavior with a single correct output given fixed inputs; they do not hold judgment, drafting, or open-ended language work.
<!-- /append -->
...
2. Always load the two companion methodology rules before proposing, writing, or revising the script body:
   - `fabric prompt get script-engineering` — clarity, structure, interface-parity, determinism-boundary, side-effect-budget, portability, testability, and improvement-synthesis layers
   - `fabric prompt get script-bug-finding` — hotspot mapping, contract extraction, branch and I/O exploration, universal bug-class sweep, counterexample construction, dynamic validation, and reporting
...
3. Anchor on the script-specific side of fabric state:
   ...
   The shared `fabric script list` / `fabric script help <id>` / `fabric prompt list` baseline is injected by the `authoring-ground-state` middleware — do not restate it here.
<!-- /append -->

<!-- append "script_scaffold_via_script" -->
4. Emit the skeleton deterministically via fabric script instead of writing `module.exports = {...}` by hand:
    - run `fabric script run script-scaffold --id <kebab-id> --name <name> --description <description> [--parameters <json>] [--returns <str>] [--examples <json>] [--details <json>] [--usage <json>] [--notes <json>]` to produce a valid starter skeleton with empty or seeded `interface` fields
    - use `fabric script help script-scaffold` when you need the full parameter list or examples
...
5. The module MUST export an object with these fields:
   ...
   - `name` — human-friendly one-line name shown in `fabric script list`
   - `description` — one-line purpose shown in `fabric script list` and `fabric script help`
   ...
   - `interface` — strongly recommended; its shape drives `fabric script help <id>`. Fields: `details: string[]`, `usage: string[]`, `parameters: { name, type, required, description }[]`, `returns: string`, `examples: { command, description }[]`, `notes: string[]`
   - all text fields in `interface` MUST match the script's actual behavior; `fabric script help` reads this object verbatim, so any drift becomes user-facing misinformation
<!-- /append -->
...
7. Determinism boundary — a fabric script MUST satisfy all of these:
...
8. Default side-effect budget — a fabric script:
...
13. Verify before registering via deterministic scripts:
    - run `fabric script run script-lint <path>` and fix every CRITICAL and HIGH finding; fabric script help output is driven by the exported `interface`, so a HIGH interface-shape finding is a real defect
    - run `fabric script run prompt-register-dryrun <path> --type script` to confirm the new file is covered by an active `script_files` glob; if `covered` is `false`, extend the target kit's `resources.toml` or choose a different directory before continuing
...
14. Output contract — follow the shared rules injected by the `authoring-output-contract` middleware below, plus these script-specific deltas:
    - print the verification command `fabric script help <id>` so the user can confirm the help output renders from the exported `interface`
    - do NOT promise that `fabric register` will generate a slash command or skill entry point for this script — per the shared contract's rule 4, scripts never become skill entry points; `fabric register` only refreshes `script_files` glob coverage so that `fabric script run <id>` and `fabric script help <id>` resolve
<!-- /append -->
