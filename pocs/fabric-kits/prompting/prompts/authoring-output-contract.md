...
4. `fabric register` ONLY generates skill entry points (CLI / agent skills) for prompts whose `type` is `skill`. See `pocs/fabric/src/register.js` `isRegistrablePromptType`. For `rules`, `template`, `middleware`, `workflow`, `checklist`, and `agent` prompts — and for fabric scripts — `fabric register` only refreshes manifest coverage; it does not produce a slash command, agent binary, or any generated artifact. Do not promise "running `fabric register` will create a skill for this" unless the authored file is a `skill` prompt.
