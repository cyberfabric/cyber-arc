...
description: Inject a conditional load of script-engineering, script-bug-finding, and kit-dependencies when the prompt under discussion invokes a fabric script
target_types: rules
...
- `fabric prompt get kit-dependencies` — per-kit third-party dependency methodology covering declaration in `package.json` + `[dependencies]`, the three install strategies, runtime reachability, and the authoring workflow via `fabric register` / `fabric kit install`. Load this whenever a script's `require` graph touches a non-Node-core, non-`@cyberfabric/fabric` package, or whenever the kit ships a `package.json` or a `[dependencies]` table.
