...
type: rules
name: prompt review
description: Review a fabric-poc prompt file or prompt kit against type conventions, marker syntax, determinism expectations, and registration scope
---

<!-- append "review_intro" -->
Use fabric-poc prompt review mode.
<!-- /append -->
...
1. Anchor on the review-specific side of fabric state:
   ...
   - for each reviewed prompt file, prefer `fabric-poc prompt source <id>` when the prompt is already registered — it returns the fully resolved source with applied middleware; otherwise read the file directly from disk
   - for each reviewed script file, read the file directly from disk, then confirm the exported contract and help surface with `fabric-poc script help <id>` when the script id is already registered
   The shared `fabric-poc prompt types` / `fabric-poc prompt list` / `fabric-poc script list` baseline is injected by the `authoring-ground-state` middleware — do not restate it here.
<!-- /append -->

<!-- append "review_deterministic_checks" -->
2. Run the deterministic checks via fabric scripts before forming opinions:
   - for a single-file review, run `fabric-poc script run prompt-lint <path>` plus `fabric-poc script run prompt-register-dryrun <path>` for a prompt file, or `fabric-poc script run script-lint <path>` plus `fabric-poc script run prompt-register-dryrun <path> --type script` for a script file
   - for a directory / kit review, run `fabric-poc script run prompt-audit <dir>` — it enumerates and sweeps every prompt and script file, returning per-file lint findings plus a summary; then run `fabric-poc script run prompt-kit-lint <kit-dir>` for cross-file defects (id collisions, orphan middleware target_prompts, broken `fabric-poc prompt get` / `fabric-poc script run` references, router-mode symmetry); after importing those findings, load `fabric-poc prompt get prompt-review-kit` for kit-level judgment criteria that sit on top
   - `fabric-poc script run prompt-lint <path>` — reports frontmatter, type, id-format, middleware field, marker-balance, insert-anchor, and unique-id findings
   - `fabric-poc script run prompt-register-dryrun <path>` — reports whether the file is covered by an active `prompt_files` glob and which manifest governs it
   - `fabric-poc script run script-lint <path>` — reports script export, interface-shape, portability, and side-effect-budget findings
   - `fabric-poc script run prompt-register-dryrun <path> --type script` — reports whether the file is covered by an active `script_files` glob and which manifest governs it
   - `fabric-poc script run prompt-audit <dir>` — aggregates the per-file lint + coverage across a kit or directory
   - `fabric-poc script run prompt-kit-lint <kit-dir>` — reports cross-file integrity findings the per-file linters cannot catch
   - use `fabric-poc script help <id>` on any of the above when you need its exact interface
...
3. Load the companion methodology rules and apply their layer maps in addition to the fabric-specific checks below:
   - always load only the smallest decisive slices needed from `fabric-poc prompt get prompt-engineering`, then walk its L1-L9 layers (document classification, clarity, structure, completeness, anti-patterns, context engineering, testability, agent ergonomics, improvement synthesis); use its severity and effort rubrics when classifying quality findings
   ...
   - when the user is asking for defect hunting, root-cause analysis, hidden-failure discovery, or safety review — or when the earlier deterministic and type-fit checks already reveal concerning behavior — also load only the smallest decisive slices needed from `fabric-poc prompt get prompt-bug-finding` and walk its L1-L7 layers (hotspot mapping, contract extraction, branch/state/handoff exploration, universal bug-class sweep, counterexample construction, dynamic validation, reporting with review status and residual risk)
...
5. Check the determinism boundary (judgment — not covered by the linter):
   ...
   - recommend moving such behavior into a fabric-poc script invoked via `fabric-poc script run <id>`, with a matching `fabric-poc script help <id>` for its interface
   - if the prompt already calls a script, confirm the script appears in `fabric-poc script list` and that the invocation command matches `fabric-poc script help <id>`
<!-- /append -->

<!-- append "review_router_pattern" -->
6. When the prompt participates in a router pattern (judgment — not covered by the linter):
   - confirm the router `skill` only dispatches and loads mode rules via `fabric-poc prompt get <router>-<mode>`
